...
CREATE TABLE PEDIDO(
	IDPEDIDO INT PRIMARY KEY,	/*CODIGO DE RASTREIO*/
	FRETE FLOAT,
	ID_PRODUTO INT,							/*FK*/
...
CREATE TABLE TIPO_FUNCIONARIO(
	IDTIPO_FUNCIONARIO INT PRIMARY KEY,
	TIPO VARCHAR(20) NOT NULL						/*MOTORISTA OU OPERADOR*/
);
...
FOREIGN KEY(ID_TIPO_FUNCIONARIO) REFERENCES TIPO_FUNCIONARIO(IDTIPO_FUNCIONARIO);
...
/*INSERINDO INFORMACOES INTERNAS*/
INSERT INTO TIPO_FUNCIONARIO VALUES(1,"OPERADOR");
INSERT INTO TIPO_FUNCIONARIO VALUES(2,"MOTORISTA");

SET GLOBAL FOREIGN_KEY_CHECKS=0;

CREATE VIEW VIEW_PEDIDOS
	AS SELECT CLI.IDCLIENTE AS 'CPF/CNPJ',CLI.NOME AS 'NOME',CLI.UF AS 'UF (C)',CLI.CEP AS 'CEP (C)',PED.FRETE AS 'FRETE',PED.IDPEDIDO AS 'CODIGO PEDIDO',PEDVEIC.ID_PLACA AS 'CAMINHAO',LOCAL.UF AS 'UF',LOCAL.CEP AS 'CEP'
	FROM PEDIDO PED
	INNER JOIN CLIENTE CLI
	ON PED.ID_CLIENTE = CLI.IDCLIENTE
	INNER JOIN PEDIDO_VEICULO PEDVEIC
	ON PED.IDPEDIDO = PEDVEIC.ID_PEDIDO
	INNER JOIN LOCAL
	ON LOCAL.UF = CLI.UF AND LOCAL.CEP = CLI.CEP;

	DELIMITER $
	CREATE PROCEDURE MAISDE10COMPRAS()
	BEGIN
		SELECT COUNT(*) AS 'PEDIDOS',PEDIDO.ID_CLIENTE AS 'CLIENTE' FROM PEDIDO
		GROUP BY(ID_CLIENTE)
		WHERE PEDIDOS > 10; 
	END
	$
